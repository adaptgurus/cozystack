param([ValidateSet('Plan')][string]$Mode = 'Plan')
$ErrorActionPreference = 'Stop'
Set-StrictMode -Version Latest

$out = Join-Path $env:RUNNER_TEMP "layersentry-dc-pod-range-plan-$env:GITHUB_RUN_ID-$env:GITHUB_RUN_ATTEMPT-$Mode"
New-Item -ItemType Directory -Path $out -ErrorAction Stop | Out-Null
$summary = Join-Path $out 'summary.json'
$state = [ordered]@{ target = '10.10.10.14'; user = 'root'; runnerCommit = $env:GITHUB_SHA; runId = $env:GITHUB_RUN_ID; runAttempt = $env:GITHUB_RUN_ATTEMPT; status = 'PENDING'; mode = $Mode; networkConfigurationRequested = ($Mode -cne 'Plan'); guestJournalWritesPossible = ($Mode -cne 'Plan') }
...
try {
    ...
    $envelope = @{ schema = 1; target = '10.10.10.14'; mode = $Mode; sources = $sources; proof = $proof.base64; labReceipts = $labReceipts; reservation = 'ROOT_EXCLUSIVE_DC_POD_RANGE_INCLUDING_ADMIN_AND_SYNCHRONOUS_IP_WRITERS';
                   apiKey = $env:CLOUDSTACK_API_KEY; apiSecret = $env:CLOUDSTACK_SECRET_KEY } | ConvertTo-Json -Depth 8 -Compress
    $loader = (Get-Content -LiteralPath 'hack/layersentry/run-dc-pod-range-plan-stdin.py' -Raw -Encoding UTF8).Replace("`r`n", "`n")
    ...
    if ($sshExit -ne 0) { throw 'Pinned-host Pod Plan failed; no range update was requested.' }
    if ($result.schema -ne 1 -or $result.target -cne '10.10.10.14' -or $result.mode -cne 'EXCLUSIVE_DISABLED_DC_LAB_ONLY' -or $result.zoneEnabled -ne $false -or $result.configurationUpdateRequested -ne $false -or $result.status -cne 'LAB_PLAN_REVIEW_REQUIRED' -or $result.capacityUsed -cne 'UNKNOWN_DISABLED_ZONE') { throw 'Pod lab Plan binding failed.' }
    $result | ConvertTo-Json -Depth 12 | Set-Content -LiteralPath (Join-Path $out 'pod-range-plan.json') -Encoding UTF8
    $state.status = 'OBSERVED'
} catch {
    # Parser/native exceptions can include remote content. Publish only state.
    throw "DC Pod Plan failed: $($state.status)."
} finally {
