...
$env:BOOT_RUN='123';$env:BOOT_RUNNER_SOURCE='a'*40;$env:BOOT_SOURCE='9dcd8ea01be5b6463488e7e9660f7106f0f4fdda';$env:CAPTURE_SOURCE='0152e63182e0f01abda723206f57325042a2536f';$env:IMAGE_SHA256='b'*64
...
$cases=@(@($summary,'runnerCommit','c'*40),@($summary,'bootSource',$env:CAPTURE_SOURCE),@($ownership,'retainForDrQualification',$false),@($ownership,'diskPath','/customer.qcow2'),@($facts,'selinux','Permissive'),@($facts,'hostPublicKeyExportVerified',$false),@($facts,'fixtureId',[Guid]::NewGuid().ToString('D')))
