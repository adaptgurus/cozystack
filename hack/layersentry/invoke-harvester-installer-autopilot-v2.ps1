...
param(
    ...
    [string]$OutputDirectory = (Join-Path $env:RUNNER_TEMP 'layersentry-installer-autopilot-v2'),
    [ValidateRange(1, 180)]
...
$v2Classifier = Join-Path $PSScriptRoot 'classify-harvester-installer-screen-v2.ps1'
$autopilot = Join-Path $PSScriptRoot 'invoke-harvester-installer-autopilot.ps1'

foreach ($path in @($baseClassifier, $v2Classifier, $autopilot)) {
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) {
        throw "Required autopilot v2 input is missing: $path"
    }
}
...
try {
    Copy-Item -LiteralPath $v2Classifier -Destination $baseClassifier -Force
    & $autopilot `
        -VmNames $VmNames `
