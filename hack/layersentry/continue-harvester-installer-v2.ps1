...
param(
    ...
    [string]$EvidenceDirectory = (Join-Path $env:RUNNER_TEMP 'layersentry-installer-orchestrator-v2'),
...
$ocrScript = Join-Path $scriptRoot 'ocr-console-v2.ps1'
...
function Get-InstallerState {
    ...
    elseif ($normalized -match 'HARVESTER.*LOGIN|WELCOME TO HARVESTER|MANAGEMENT URL|KUBERNETES.*READY') {
    ...
    elseif ($normalized -match 'ARE YOU SURE|DO YOU WANT TO CONTINUE|PROCEED WITH INSTALLATION|START INSTALLATION') {
        ...
        $reason = 'Installer requests yes or continue confirmation.'
    }
...
function Get-OcrText {
    ...
    Assert-Condition -Condition (Test-Path -LiteralPath $image -PathType Leaf) -Message "Console image is missing for $VmName: $image"
    & $ocrScript -ImagePath $image -OutputPath $ocrPath
    $ocr = Get-Content -LiteralPath $ocrPath -Raw -Encoding UTF8 | ConvertFrom-Json
    if ($ocr.PSObject.Properties['error']) {
        if (-not [string]::IsNullOrWhiteSpace([string]$ocr.error)) {
...
function Set-InstalledBootConfiguration {
    ...
    Assert-Condition -Condition ($disks.Count -ge 1) -Message "$VmName has no attached virtual hard disk."
...
foreach ($path in @($captureScript, $commandScript, $ocrScript, $credentialPath)) {
    Assert-Condition -Condition (Test-Path -LiteralPath $path -PathType Leaf) -Message "Required installer automation input is missing: $path"
}
foreach ($vmName in $VmNames) {
    Assert-Condition -Condition ($null -ne (Get-VM -Name $vmName -ErrorAction SilentlyContinue)) -Message "Required VM does not exist: $vmName"
}
...
try {
    while ((Get-Date) -lt $deadline) {
        ...
        $vip443 = Test-Port -Address $ClusterVip -Port 443
        $vip6443 = Test-Port -Address $ClusterVip -Port 6443
        $sen1Ssh = Test-Port -Address '10.10.10.11' -Port 22
        $clusterReady = $vip443 -or ($vip6443 -and $sen1Ssh)
        $states = New-Object System.Collections.Generic.List[object]
        $madeProgress = $false

        foreach ($vmName in $VmNames) {
            $text = Get-OcrText -VmName $vmName -CaptureDirectory $captureDirectory -IterationDirectory $iterationDirectory
            $detected = Get-InstallerState -VmName $vmName -Text $text
            $state = [string]$detected.state
            $states.Add($detected)

            if (-not $repeatCount.ContainsKey($vmName)) {
            ...
            Write-Host ("iteration={0} vm={1} state={2} repeat={3} clusterReady={4}" -f $iteration, $vmName, $state, $repeatCount[$vmName], $clusterReady)
            ...
            $actions = @(Get-ActionForState -State $state -AllowInstallation $allowInstallation)
            if ($actions.Count -gt 0) {
                ...
                Invoke-ApprovedAction -VmName $vmName -Actions $actions -RequestId $requestId -IterationDirectory $iterationDirectory
                $madeProgress = $true
            }
        ...
        $iterationState = [ordered]@{
            ...
            vip443Reachable = $vip443
            vip6443Reachable = $vip6443
            sen1SshReachable = $sen1Ssh
            states = @($states)
            seenInstalling = $seenInstalling
            bootPrepared = $bootPrepared
        }
        Write-JsonFile -Value $iterationState -Path (Join-Path $iterationDirectory 'state.json')
