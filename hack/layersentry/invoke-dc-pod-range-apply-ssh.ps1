param([ValidateSet('Apply','Observe')][string]$Mode = 'Observe')
$ErrorActionPreference = 'Stop'
Set-StrictMode -Version Latest

$out = Join-Path $env:RUNNER_TEMP "layersentry-dc-pod-range-action-$env:GITHUB_RUN_ID-$env:GITHUB_RUN_ATTEMPT-$Mode"
New-Item -ItemType Directory -Path $out -ErrorAction Stop | Out-Null
$summary = Join-Path $out 'summary.json'
$state = [ordered]@{ target = '10.10.10.14'; user = 'root'; runnerCommit = $env:GITHUB_SHA; runId = $env:GITHUB_RUN_ID; runAttempt = $env:GITHUB_RUN_ATTEMPT; status = 'PENDING'; mode = $Mode; networkConfigurationRequested = ($Mode -ceq 'Apply'); guestJournalWritesPossible = ($Mode -ceq 'Apply') }
...
try {
    ...
    $sources = @{}
    foreach ($entry in @(@('dr_recovery_acceptance','dr_recovery_acceptance.py'), @('dc_native_storage_registration','register-dc-native-storage.py'), @('dc_storage_loader','run-dc-storage-registration-stdin.py'), @('dc_pod_range','dc-pod-range.py'))) {
    ...
    $labReceipts=@{}
    foreach($name in @('capacity','guestNetwork','hypervNetwork')){
        $labReceipts[$name]=[Convert]::ToBase64String([IO.File]::ReadAllBytes((Join-Path 'hack/layersentry/evidence' "lab-pod-reviewed-$name.json")))
    }
    # Credentials are sent exclusively through encrypted SSH stdin. They never enter command arguments or disk files.
    $envelope = @{ schema = 1; target = '10.10.10.14'; mode = $Mode; sources = $sources; proof = $proof.base64; reviewedPlan = [Convert]::ToBase64String([IO.File]::ReadAllBytes('hack/layersentry/evidence/dc-pod-reviewed-plan.json'));  labReceipts = $labReceipts; reservation = 'ROOT_EXCLUSIVE_DC_POD_RANGE_INCLUDING_ADMIN_AND_SYNCHRONOUS_IP_WRITERS';
                   apiKey = $env:CLOUDSTACK_API_KEY; apiSecret = $env:CLOUDSTACK_SECRET_KEY } | ConvertTo-Json -Depth 8 -Compress
    $loader = (Get-Content -LiteralPath 'hack/layersentry/run-dc-pod-range-apply-stdin.py' -Raw -Encoding UTF8).Replace("`r`n", "`n")
    ...
    if ($null -ne $result.PSObject.Properties['journalOperation']) {
        $op=$result.journalOperation
        if ($op.state -cnotin @('NOT_SUBMITTED','SUBMITTING','ASYNC_SUBMITTED','SUBMISSION_UNCERTAIN','RECONCILED')) { throw 'Invalid public journal state.' }
        $public=[ordered]@{state=[string]$op.state}
        if ($null -ne $op.PSObject.Properties['jobId']) {
            if($op.jobId -cnotmatch '^[0-9a-f]{8}(-[0-9a-f]{4}){3}-[0-9a-f]{12}$'){throw 'Invalid public job identity.'}
            $public['jobId']=[string]$op.jobId
        }
        $state['journalOperation']=$public
    }
    if ($sshExit -ne 0) { throw 'Pinned-host Pod action failed; inspect recorded intent before any further operation.' }
    if ($result.schema -ne 1 -or $result.target -cne '10.10.10.14' -or $result.mode -cne 'EXCLUSIVE_DISABLED_DC_LAB_ONLY' -or $result.zoneEnabled -ne $false -or $result.configurationUpdateRequested -ne ($Mode -ceq 'Apply') -or $result.phase -cne $Mode -or $result.capacityUsed -cne 'UNKNOWN_DISABLED_ZONE' -or $result.reviewedPlanSha256 -cne '240e1b1f9c28e703643ef21d0cf4aecb01ac87ccc4b166978e19a8b0b5bdf665' -or $result.automaticReplay -ne $false) { throw 'Pod lab action binding failed.' }
    if (($Mode -ceq 'Apply' -and $result.status -cne 'LAB_RANGE_RECONCILED_USAGE_UNKNOWN_NOT_SYSTEMVM_READY') -or ($Mode -ceq 'Observe' -and $result.status -cnotin @('LAB_RANGE_OBSERVED_USAGE_UNKNOWN_NOT_SYSTEMVM_READY','LAB_RANGE_OLD_OBSERVED_NO_AUTOMATIC_REPLAY'))) {throw 'Pod lab action status failed.'}
    $result | ConvertTo-Json -Depth 12 | Set-Content -LiteralPath (Join-Path $out 'pod-range-action.json') -Encoding UTF8
    $state.status = 'OBSERVED'
} catch {
    # Parser/native exceptions can include remote content. Publish only state.
    throw "DC Pod action failed: $($state.status)."
} finally {
