...
The recovery Zone must use Advanced networking. Exact 4.22.1.1 `UserVmManagerImpl.allocateVMFromBackup` populates a network ID list from backup metadata when network inputs are omitted (lines 9774–9784), while `createVirtualMachine` rejects any non-null network ID list for a Basic destination (lines 6531–6533). Explicit `iptonetworklist` does not avoid this: `BaseDeployVMCmd.getNetworkIds` converts it into a list (lines 501–518). Basic deployment normally chooses its one system-owned Shared Guest network implicitly (`NetworkModelImpl.getExclusiveGuestNetwork`, line 1471). Merely relaxing this adapter's account-owned-network check cannot fix the native create-from-backup path. The preflight rejects Basic destinations before any mutation. A Basic source and Advanced destination can satisfy the source-derived contract, subject to live proof; two Basic Zones do not meet this adapter's native recovery prerequisite. This is a source finding, not a live negative result.
