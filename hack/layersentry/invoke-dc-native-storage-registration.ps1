param([ValidateSet('Plan','Register')][string]$Mode = 'Plan')
$ErrorActionPreference = 'Stop'
Set-StrictMode -Version Latest

$out = Join-Path $env:RUNNER_TEMP "layersentry-dc-storage-registration-$env:GITHUB_RUN_ID-$env:GITHUB_RUN_ATTEMPT"
New-Item -ItemType Directory -Path $out -ErrorAction Stop | Out-Null
$summary = Join-Path $out 'summary.json'
$state = [ordered]@{ target = '10.10.10.14'; user = 'root'; runnerCommit = $env:GITHUB_SHA; runId = $env:GITHUB_RUN_ID; runAttempt = $env:GITHUB_RUN_ATTEMPT; status = 'PENDING'; mode = $Mode; registrationRequested = ($Mode -ceq 'Register'); guestJournalWritesPossible = $true }
...
try {
    ...
    $sources = @{}
    foreach ($entry in @(@('dr_recovery_acceptance','dr_recovery_acceptance.py'), @('dc_native_storage_registration','register-dc-native-storage.py'))) {
    ...
    if ($sshExit -ne 0) { throw 'Pinned-host registration failed; inspect durable journal before any next execution.' }
    if ($result.schema -ne 1 -or $result.target -cne '10.10.10.14' -or $result.executeRequested -ne ($Mode -ceq 'Register') -or $result.zoneEnabled -ne $false -or $result.preservedPool -cne '9c9fbd8f-e4ee-4e02-9767-6d1cc7a2b8c9') { throw 'Registration evidence binding failed.' }
    $result | ConvertTo-Json -Depth 12 | Set-Content -LiteralPath (Join-Path $out 'registration.json') -Encoding UTF8
    $state.status = 'OBSERVED'
} catch {
    # Parser/native exceptions can include remote content. Publish only state.
    throw "DC native storage registration failed: $($state.status)."
} finally {
