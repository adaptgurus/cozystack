...
## ProgramData startup correction and credential-free regression

Diagnostic run34062939084 spawned exact Windows SSH PID4600 and exited255 with no spawn error, UNKNOWN stderr class and verified cleanup. Primary Windows OpenSSH v9.5.0.0 `contrib/win32/win32compat/w32fd.c:213` calls `_dupenv_s(..., "ProgramData")` in `init_prog_paths()` and fatals with a fixed missing-variable message when absent, before standard-FD initialization and before SSH main/config processing. The old six-variable child environment omitted ProgramData. Both DC and DR now use the same seven-variable `sshEnvironment()` allowlist preserving ProgramData without inheriting credentials, SSH agents or debug variables. Exact [startup implementation](https://github.com/PowerShell/openssh-portable/blob/v9.5.0.0/contrib/win32/win32compat/w32fd.c) supplies the new fixed-message diagnostic categories; actual Windows regression remains required.

Root may invoke `node hack/layersentry/gui-acceptance/diagnose-ssh-startup.mjs <existing-ACL-private-directory> <new-public-receipt.json>` with `LAYERSENTRY_GUI_ACL_VERIFIED=1` after verifying the private directory ACL, and no secret environment injection. The helper runs the same exact System32/OpenSSH executable with configuration-only `-G -F NUL`, fixed .14 IP, canonicalization disabled, no proxy/local command and no forward. It compares the previous environment against an otherwise identical environment containing ProgramData. It then runs the exact askpass helper using a fixed public marker through a fixed PowerShell invocation under both environments. Each child has a10s deadline and bounded private output capture; only exit/spawn/signal categories, dummy equality and child-close status enter the receipt. The helper creates/removes only its uniquely named dummy file. PASS requires baseline exit255, corrected exit0, identical executable bytes/path, arguments, stdio and every environment entry except ProgramData, plus both dummy marker successes and verified process completion. A baseline exact PROGRAMDATA_MISSING message gives EXACT_MESSAGE_AND_DIFFERENTIAL proof; UNKNOWN is accepted only with zero stderr bytes and records DIFFERENTIAL_ONLY_EARLY_STDERR_UNAVAILABLE. Other exit codes, known errors, unclassified nonempty stderr or changed invariants block. Any different cause, timeout or uncertain cleanup blocks the gate; no password or network retry occurs. This validates local startup/helper behavior only, not actual SSH authentication, forwarding, browser trust or production readiness.

Twelve focused Node tests pass for the shared allowlist, credential exclusion, configuration-only arguments, fixed-message classifier and strict diagnostic gate, together with the existing credential/listener guard tests. Windows execution is delegated to root's separately reviewed runner step; no workflow, live dispatch or secret retrieval was performed by this source change.


Exact early-output analysis: `fatal.c:sshfatal()` calls `sshlogv()` then `cleanup_exit(255)`; `log.c` writes to FD2, Win32 `inc/unistd.h` maps that call to `w32_write()`, and `w32fd.c:CHECK_FD` returns EBADF while the global descriptor table is still empty. Because ProgramData validation precedes `fd_table_initialize()`, this specific missing-variable fatal cannot rely on a readable message. The revised diagnostic retains actual stderr byte counts/classification and distinguishes silent differential proof from observed exact-message proof. Each launch records hashes of executable bytes/path, fixed arguments, stdio and all environment values except ProgramData; acceptance compares those hashes and requires ProgramData absent then present. No raw environment value, executable output or credential is published. Three focused diagnostic tests pass, including zero-byte early failure acceptance and rejection of baseline0/1, unrelated errors, nonempty UNKNOWN, changed executable/arguments/environment and failed dummy/cleanup checks. This correction has no live execution.


## Closed-pipe candidate after local startup observation

No-network run34063539296 establishes the ProgramData differential: old SSH environment exits255 with zero stderr bytes; corrected environment exits0 with zero stderr and identical other input bindings. Both PowerShell dummy-helper invocations nevertheless exit0 with zero stdout and no marker, so the complete gate remained BLOCKED and no real authentication ran. NUL/ignored stdin is a hypothesis for that second failure, not yet a proven PowerShell cause. Current open-source PowerShell is not substituted for the runner's Windows PowerShell5.1 internals.

The helper now retains those NUL results and adds a stdin-only comparison under the fixed environment: the same PowerShell executable bytes/path and exact helper arguments/environment receive a valid pipe closed immediately without writing data. The exact SSH configuration-only invocation is also checked with a closed pipe. PASS requires ProgramData proof independently, both NUL helper baselines to reproduce empty output, the closed-pipe helper to emit the exact dummy marker, configuration-only SSH exit0, matching executable/arguments/full-environment hashes across each stdin-only pair, no timeout/truncation and all children closed. `causalProof` now separately records `programData` and `stdin` scope. The shared DC/DR launcher contains the closed-pipe candidate (`child.stdin.end()` immediately); root must require this no-secret diagnostic PASS before any live use of that candidate. No password or shell command enters stdin. Twelve focused tests pass, including changed-input, absent-marker, bad-exit and uncertain-cleanup rejection. No runtime or workflow change was dispatched here.
