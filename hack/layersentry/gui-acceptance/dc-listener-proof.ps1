...
try {
  ...
  if($ExpectAbsent) {
    ...
    @{schema=1;listenerAbsent=$true;localLoopbackPort=$LocalPort}|ConvertTo-Json -Compress
    exit 0
  }
  ...
  @{schema=1;target='dc';sshHost='10.10.10.14';localLoopbackPort=$LocalPort;remoteLoopbackPort=8080;processId=$SshProcessId;processStartedAt=$start;listenerOwnerVerified=$true;processPathVerified=$true}|ConvertTo-Json -Compress
} catch {
  # Process inspection errors may include local state: emit a fixed public code only.
  [Console]::Error.WriteLine('DC_LISTENER_PROOF_FAILED')
  exit 1
}
