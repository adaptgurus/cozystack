...
if(-not $module.Contains('[string]$_.VMName -ine $Request.vmName')){throw 'All-adapter occupancy must exclude only the uniquely named owned VM.'}
if(-not $module.Contains('[string]$route.InterfaceAlias -ieq $desiredAlias')){throw 'Owned Windows interface routes must tolerate alias casing.'}
