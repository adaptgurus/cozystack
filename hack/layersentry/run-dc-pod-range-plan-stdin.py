...
def parse_payload(raw):
    ...
    data = json.loads(raw)
    if set(data) != {'schema', 'target', 'mode', 'sources', 'proof', 'apiKey', 'apiSecret', 'labReceipts', 'reservation'}:
        raise ValueError('INPUT_FIELDS')
    if data['schema'] != 1 or data['target'] != '10.10.10.14' or data['mode'] != 'Plan':
...
def main():
    ...
    try:
        ...
        # This loader exposes only the read-only lab Plan, never Apply or journal creation.
        result = network.execute_lab(api, receipts, payload['reservation'])
