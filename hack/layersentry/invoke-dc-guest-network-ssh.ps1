param([ValidateSet('Plan','Bridge','Label')][string]$Mode = 'Plan')
$ErrorActionPreference = 'Stop'
Set-StrictMode -Version Latest

$out = Join-Path $env:RUNNER_TEMP "layersentry-dc-guest-network-$env:GITHUB_RUN_ID-$env:GITHUB_RUN_ATTEMPT-$Mode"
...
try {
    ...
    $sources = @{}
    foreach ($entry in @(@('dr_recovery_acceptance','dr_recovery_acceptance.py'), @('dc_native_storage_registration','register-dc-native-storage.py'), @('dc_storage_loader','run-dc-storage-registration-stdin.py'), @('dc_guest_network','dc-guest-network.py'))) {
    ...
    $loader = (Get-Content -LiteralPath 'hack/layersentry/run-dc-guest-network-stdin.py' -Raw -Encoding UTF8).Replace("`r`n", "`n")
    ...
    if ($sshExit -ne 0) { throw 'Pinned-host guest network phase failed; inspect durable journal before any next execution.' }
    if ($result.schema -ne 1 -or $result.target -cne '10.10.10.14' -or $result.phase -cne $Mode -or $result.zoneEnabled -ne $false -or $result.managementPreserved -ne $true -or $result.bridge -cne 'ls-guest0' -or $result.guestMac -cne '02:29:ba:17:6b:81') { throw 'Guest network evidence binding failed.' }
    $result | ConvertTo-Json -Depth 12 | Set-Content -LiteralPath (Join-Path $out 'guest-network.json') -Encoding UTF8
    $state.status = 'OBSERVED'
} catch {
    # Parser/native exceptions can include remote content. Publish only state.
    throw "DC guest network phase failed: $($state.status)."
} finally {
